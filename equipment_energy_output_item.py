...
def main(logger):
    ...
    while True:
        ...
        try:
            ...
            logger.error("Error in step 1.1 of equipment_energy_output_item.main " + str(e))
            if cursor_system_db:
        ...
        try:
            ...
            logger.error("Error in step 1.2 of equipment_energy_output_item.main " + str(e))
            # sleep and continue the outer loop to reconnect the database
            time.sleep(60)
            continue
        finally:
...
def worker(equipment):
    ...
    try:
        ...
        error_string = "Error in step 1.1 of equipment_energy_output_item.worker " + str(e)
        if cursor_system_db:
            cursor_system_db.close()
        if cnx_system_db:
            cnx_system_db.close()
    ...
    try:
        ...
        error_string = "Error in step 1.2 of equipment_energy_output_item.worker " + str(e)
        if cursor_system_db:
            cursor_system_db.close()
        if cnx_system_db:
            cnx_system_db.close()
    ...
    try:
        ...
        error_string = "Error in step 2.1 of equipment_energy_output_item.worker " + str(e)
        if cursor_system_db:
            cursor_system_db.close()
        if cnx_system_db:
            cnx_system_db.close()
    ...
    try:
        ...
        error_string = "Error in step 3.1 of equipment_energy_output_item.worker " + str(e)
        print(error_string)
        return error_string
    finally:
        if cursor_system_db:
    ...
    try:
        ...
        error_string = "Error in step 4.1 of equipment_energy_output_item.worker " + str(e)
        if cursor_energy_db:
    ...
    try:
        ...
        error_string = "Error in step 4.2 of equipment_energy_output_item.worker " + str(e)
        if cursor_energy_db:
            cursor_energy_db.close()
        if cnx_energy_db:
            cnx_energy_db.close()
    ...
    try:
        ...
        error_string = "Error in step 5.1 of equipment_energy_output_item.worker " + str(e)
        if cursor_energy_db:
            cursor_energy_db.close()
        if cnx_energy_db:
            cnx_energy_db.close()
    ...
    if virtual_meter_list is not None and len(virtual_meter_list) > 0:
        try:
            ...
            error_string = "Error in step 6.1 of equipment_energy_output_item.worker " + str(e)
            if cursor_energy_db:
                cursor_energy_db.close()
            if cnx_energy_db:
                cnx_energy_db.close()
    ...
    if offline_meter_list is not None and len(offline_meter_list) > 0:
        try:
            ...
            error_string = "Error in step 7.1 of equipment_energy_output_item.worker " + str(e)
            if cursor_energy_db:
                cursor_energy_db.close()
            if cnx_energy_db:
                cnx_energy_db.close()
    ...
    try:
        ...
        error_string = "Error in step 9 of equipment_energy_output_item.worker " + str(e)
        if cursor_energy_db:
            cursor_energy_db.close()
        if cnx_energy_db:
            cnx_energy_db.close()
    ...
    if len(aggregated_values) > 0:
        try:
            add_values = (" INSERT INTO tbl_equipment_output_item_hourly "
                          "             (equipment_id, "
                          "              energy_item_id, "
                          "              start_datetime_utc, "
                          "              actual_value) "
            ...
            error_string = "Error in step 10.1 of equipment_energy_output_item.worker " + str(e)
            print(error_string)
            return error_string
        finally:
            if cursor_energy_db:

...
#   Step 7: aggregate energy data in the common time slot by energy categories and hourly
...
def worker(space):
    ...
    try:
        ...
        error_string = "Error in step 2 of space_energy_output_category.worker " + str(e)
        print(error_string)
        return error_string
    finally:
        if cursor_system_db:
    ...
    try:
        ...
        error_string = "Error in step 3.1 of space_energy_output_category.worker " + str(e)
        if cursor_energy_db:
    ...
    try:
        ...
        error_string = "Error in step 3.2 of space_energy_output_category.worker " + str(e)
        if cursor_energy_db:
            cursor_energy_db.close()
        if cnx_energy_db:
            cnx_energy_db.close()
    ...
    if equipment_list is not None and len(equipment_list) > 0:
        try:
            ...
            error_string = "Error in step 4 of space_energy_output_category.worker " + str(e)
            if cursor_energy_db:
                cursor_energy_db.close()
            if cnx_energy_db:
                cnx_energy_db.close()
    ...
    print("Step 7: aggregate energy data in the common time slot by energy categories and hourly")

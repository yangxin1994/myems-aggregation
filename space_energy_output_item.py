...
def main(logger):
    ...
    while True:
        ...
        try:
            ...
            logger.error("Error in step 1.1 of space_energy_output_item.main " + str(e))
            if cursor_system_db:
        ...
        try:
            ...
            logger.error("Error in step 1.2 of space_energy_output_item.main " + str(e))
            # sleep and continue the outer loop to reconnect the database
            time.sleep(60)
            continue
        finally:
...
########################################################################################################################
# PROCEDURES:
#   Step 1: get all equipments associated with the space
#   Step 2: get all child spaces associated with the space
#   Step 3: determine start datetime and end datetime to aggregate
#   Step 4: for each equipment in list, get energy output data from energy database
#   Step 5: for each child space in list, get energy output data from energy database
#   Step 6: determine common time slot to aggregate
#   Step 7: aggregate energy data in the common time slot by energy items and hourly
#   Step 8: save energy data to energy database
#
...
def worker(space):
    ...
    # Step 1: get all equipments associated with the space
    ####################################################################################################################
    print("Step 1: get all equipments associated with the space")
    cnx_system_db = None
    cursor_system_db = None
    try:
        cnx_system_db = mysql.connector.connect(**config.myems_system_db)
        ...
        error_string = "Error in step 1.1 of space_energy_output_item.worker " + str(e)
        if cursor_system_db:
            cursor_system_db.close()
        if cnx_system_db:
            cnx_system_db.close()
    ...
    try:
        ...
        error_string = "Error in step 1.2 of space_energy_output_item.worker " + str(e)
        print(error_string)
        return error_string
    finally:
        if cursor_system_db:
    ...
    # Step 2: get all child spaces associated with the space
    ####################################################################################################################
    print("Step 2: get all child spaces associated with the space")
    ...
    try:
        ...
        error_string = "Error in step 2 of space_energy_output_item.worker " + str(e)
        print(error_string)
        return error_string
    finally:
        if cursor_system_db:
    ...
    # Step 3: determine start datetime and end datetime to aggregate
    ####################################################################################################################
    print("Step 3: determine start datetime and end datetime to aggregate")
    cnx_energy_db = None
    ...
    try:
        ...
        error_string = "Error in step 3.1 of space_energy_output_item.worker " + str(e)
        if cursor_energy_db:
    ...
    try:
        query = (" SELECT MAX(start_datetime_utc) "
                 " FROM tbl_space_output_item_hourly "
                 " WHERE space_id = %s ")
        ...
        error_string = "Error in step 3.2 of space_energy_output_item.worker " + str(e)
        if cursor_energy_db:
            cursor_energy_db.close()
        if cnx_energy_db:
            cnx_energy_db.close()
        ...
        try:
            for equipment in equipment_list:
                ...
                query = (" SELECT start_datetime_utc, energy_item_id, actual_value "
                         " FROM tbl_equipment_output_item_hourly "
            ...
            error_string = "Error in step 4 of space_energy_output_item.worker " + str(e)
            if cursor_energy_db:
                cursor_energy_db.close()
            if cnx_energy_db:
                cnx_energy_db.close()
    ...
    if child_space_list is not None and len(child_space_list) > 0:
        try:
            ...
            error_string = "Error in step 5 of space_energy_output_item.worker " + str(e)
            if cursor_energy_db:
                cursor_energy_db.close()
            if cnx_energy_db:
                cnx_energy_db.close()
    ...
    print("Step 7: aggregate energy data in the common time slot by energy items and hourly")
    ...
    try:
        ...
        error_string = "Error in step 7 of space_energy_output_item.worker " + str(e)
        if cursor_energy_db:
            cursor_energy_db.close()
        if cnx_energy_db:
            cnx_energy_db.close()
    ...
    # Step 8: save energy data to energy database
    ####################################################################################################################
    print("Step 8: save energy data to energy database")

    if len(aggregated_values) > 0:
        try:
            add_values = (" INSERT INTO tbl_space_output_item_hourly "
                          "             (space_id, "
                          "              energy_item_id, "
                          "              start_datetime_utc, "
                          "              actual_value) "
            ...
            error_string = "Error in step 8 of space_energy_output_item.worker " + str(e)
            print(error_string)
            return error_string
        finally:
            if cursor_energy_db:
